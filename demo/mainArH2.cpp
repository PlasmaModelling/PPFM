...
int main() {
      ...
      // Set calculation from electronic configuration of H
      ...
      // as algorithm is selected recognizing monotonic potentials 

      // Ar - Ar+ 
      cibox[1]->Load(false);
      auto arari = cibox[1]->GetIntInterface();

      // Multi-state interaction, setting the states
      cibox[1]->MultiState( { 2. , 4. } ); 
      ...
      // Iter to modify 2nd Potential in the MultiPot approach 
      // for the very particular case of H-H+ with Threshold CS 
      // ONLY for the second state. Setting the states...
      cibox[19]->MultiState( { 2., 2. } );

      // getting the interface
      InteractionInterface* i = cibox[19]->GetIntInterface();

      // Very particular cases like this demonstrates PPFM flexibility !
      // Editable TCS calculator
      auto p = dynamic_cast<MultiCs*>( cibox[19]->TCScalculator );

      // Potential for the 1st state
      (*p)[0] = new AvrgChiIntegrator(
          i, new Morse2Param ( 51.75 , 1.677 )
      );

      // Potential for the 2nd state
      (*p)[1] = new ThresholdCs ( i, 
          { 
      ...
      // Now adding the Charge Transfer to H - H+ interaction
      cibox[19]->TCScalculator = new CsHolder ( 

          // passing the TCS calculator for the elastic collision
          p, new ChargeTransferCs ( i, 28.69, 1.3 ) 

      );

      // H-e- Integration of Momentum Transfer Cross Section 
      cibox[21]->Load(false); 
      cibox[21]->LoadElastic("edit");
