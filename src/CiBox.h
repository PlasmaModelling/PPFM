...
/** 
 * @file CiBox.h
 * @brief This file contains the declaration of the CiBox class.
*/
...
class CiBox {
  ...
  */
   ...
   * info on the interactions stored can be obtained by calling the info() method.
  ...
  */
   ...
   * @param i-th component of class member integrals  
   * @details Needed values of the collision integrals for the 4th order
   * approximation to transport properties are : \n
   * (1,1),..,(1,7) \n
   * (2,2),..,(2,6) \n
   * (3,3),..,(3,5) \n
   * (4,4)     
  ...
  */
  ...
  /**
   * @brief Prints DeflectionAngles to CSV files for each interaction in CiBox
   * @param Ti Vector of temperatures
   * @param gasmix Pointer to the gas mixture
   * @param folder Output folder for CSV files
   * @details For each HybridInterface in CiBox, a DeflectionAngleCsv is created 
   * and used to print the corresponding angles at the given temperatures. 
  */
  void PrintDeflectionAngles ( const std::vector<double>& Ti, GasMixture* gasmix, const std::string& folder ) ;
  ...
  */ 
  ...
  /// @brief Returns the mixture components and its interactions. 
  void info() ; 

  /// @brief All the Collision Integrals will be loaded by raw data files.
  void loadAll( bool b ) ; 
