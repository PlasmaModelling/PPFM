...
class Composition {
    ...
    /// @brief  Enumerated type for selecting Debye length formulation.
    ...
    */
    ...
    /// @brief Get the indices of the base species in the mixture.
    virtual std::vector<int> getBaseIndicesB() = 0 ;

    /// @brief Get the indices of the non-base species in the mixture.
...
class GodinTrepSahaSolver : public Composition {
    ...
    /** @brief Solve the equilibrium composition with the specified algorithm in this class reference
    ...
    */
    ...
    /// @brief Interface realization for Godin–Trépanier algorithm, returns the reaction matrix v. 
    std::vector<std::vector<double>> getReactionMatrixV() override {return v;}

    /// @brief Interface realization for Godin–Trépanier algorithm, returns the indices of the base species in the mixture. 
    std::vector<int> getBaseIndicesB() override {return b;}
    
    /// @brief Interface realization for Godin–Trépanier algorithm, returns the indices of the non-base species in the mixture.
    ...
    /// @brief Matrixes required for the algorithm. Base and non-base matrices, B^-1 and reaction matrix v.
    std::vector<std::vector<double>> B,Bs,Binv,v ; 

    /// @brief Base and non-base species indices, as described in the reference.
    std::vector<int> b, bs;
    ...
    */
     ...
     * polimorphically casting its nature. 
     * @see Species.h for definitions of chemical species. 
    */
    ...
    /// @brief Re-initialize the class from scratch.
...
class GTSahaDHcorrection : public GodinTrepSahaSolver {
    ...
    /// @brief Re-initialize the class from scratch, including resetting any DH-specific state.
    virtual void restart() override;
...
class CompositionLoader : public GodinTrepSahaSolver, public DataLoader {
    ...
    /// @brief Temperature grid for which composition is loaded.
    std::vector<double> Tgrid ; 

    /// @brief ni(T) values read from file, stored as a vector of vectors (one per species).
    std::vector<std::vector<double>> nFunctions ;
    
    /// @brief Helper function to read raw data from a file into a vector of vectors.
    ...
    /// @brief Override the Init() method in @class DataLoader . 
    void Init() override ;

    /** @brief Override the BuildFileName() method in 
     * @class DataLoader to construct the filename for composition data. 
    */
    std::string BuildFileName(const std::string& name) override;

    /** 
     * @brief Override the ParseFile() method in @class DataLoader to read composition data
     * from a file and populate Tgrid and nFunctions. 
    */
    ...
    /// @brief Constructor that initializes the CompositionLoader with a specified filename for loading composition data.
    CompositionLoader(Mixture* mix, Gas* gas, const std::string& filename);

    /// @brief Constructor that initializes the CompositionLoader with a specified PfBox and filename for loading composition data.
    CompositionLoader(Mixture* mix, Gas* gas, PfBox* qbox, const std::string& filename);

    /// @brief Solve the composition including Debye–Hückel corrections.
    /// @param lambda Debye length to be used for corrections.
    void CompositionSolve(std::optional<double> lambda) override ;

    /// @brief Re-initialize the class from scratch, including resetting any loaded data.
