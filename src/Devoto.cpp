    ...
    void DevotoTP::lightComputeTransport ( GasMixture* gasmix ) {
    ...
    double DevotoTP::ThermalCondHeavy(GasMixture* gasmix, int order){
        ...
        int i, j, m, p;
        int N_ORDS = order;
        int N_Q = N_ORDS * N_SPC;

        std::vector<std::vector<double>> q(N_Q, std::vector<double>(N_Q, 0.0));
        ...
        std::vector<double> r(N_Q, 0.0);

        m = 1;
        for (i = 0; i < (N_SPC - 1); i++)   // electron last
            c[m * N_SPC + i] = n[i];

        p = 1;
        for (j = 0; j < (N_SPC - 1); j++)   // electron last
            r[p * N_SPC + j] = n[j] / sqrt(mass[j]);

        std::vector<double> y(N_Q, 0.0);
        std::vector<double> rhs = c;

        lu_sistema(y, q, rhs, N_Q);

        double quad = 0.0;
        for (i = 0; i < N_Q; i++)
            quad += r[i] * y[i];

        double tcond = (75.0 / 8.0) * kB * sqrt(2.0 * PI * kB * T) * quad;
    ...
    double DevotoTP::Viscosity(GasMixture* gasmix, int order) {
        ...
        std::vector<std::vector<double>> qcap(N_Q, std::vector<double>(N_Q, 0.0));
        ...
        lu_sistema(y, qcap, rhs, N_Q);

        double quad = 0.0;
        for (i = 0; i < N_Q; i++)
            quad += r[i] * y[i];

        double visc = (5.0 / 2.0) * sqrt(2.0 * PI * kB * T) * quad;
    ...
    void DevotoLteLambdaR::computeTransport(GasMixture* gasmix){
        ...
        Thermodynamics* thermo = new Thermodynamics();
        thermo->computeThermodynamics(*gasmix) ; 
            
        DevotoTP::lightComputeTransport(gasmix) ; 
        ...
        // precompute pairwise Dbin for all k<l
        std::vector<std::vector<double>> Dbin(N, std::vector<double>(N, 0.0));
        #pragma omp parallel for collapse(2)
        for (int k=0; k<N; ++k)
            for (int l=0; l<N; ++l)
                Dbin[k][l] = Dbinij(k,l,gasmix);
            ...
            for (size_t j = 0; j < L; j++)
                for (size_t k = 0; k < N-1; k++)
                    for (size_t l = k+1; l < N; l++)
                        A[i][j] += ((1.)/( ntot*Dbin[k][l] ) ) * x[k] * x[l] * ((vv[i][k]/x[k])-(vv[i][l]/x[l])) * ((vv[j][k]/x[k])-(vv[j][l]/x[l]));
